...
<body class="min-h-screen flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-md bg-card brutal-border brutal-shadow p-8 flex flex-col gap-6">
        ...
        <div>
            <h1 class="mono text-2xl font-black uppercase tracking-tight">Reset Password</h1>
            <p class="text-xs text-muted-foreground uppercase mono tracking-widest mt-1">Get temporary credentials via email</p>
        </div>
        ...
        <!-- Form -->
        <form action="<?= base_url('reset-password') ?>" method="POST" class="flex flex-col gap-4">
            <?= csrf_field() ?>
            <div class="flex flex-col gap-1.5">
                <label for="email" class="mono text-[10px] uppercase tracking-widest font-black text-foreground">Registered Email</label>
                <input type="email" name="email" id="email" required value="<?= old('email') ?>" class="w-full bg-background border border-foreground px-3 py-2.5 mono text-xs brutal-border focus:outline-none focus:ring-1 focus:ring-primary" placeholder="user@nexus.com">
            </div>
        ...
        <div class="border-t border-foreground pt-4 flex justify-between items-center text-[10px] mono uppercase tracking-widest">
            ...
            <a href="<?= base_url('login') ?>" class="font-black underline hover:text-primary">Sign In</a>
        </div>
    </div>
